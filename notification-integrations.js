...
// Example 1: Send SOS alert to nearby users
/*
const nearbyUsers = [
  { phone: '+1234567890', distance: 0.5 },
  { phone: '+1234567891', distance: 0.8 }
];

const whatsappPromises = nearbyUsers.map(user => 
  sendWhatsAppAlert(user.phone, 'sos_alert', {
    location: 'Central Park, NYC',
    distance: user.distance.toFixed(1),
    message: 'Large crowd stampede reported'
  })
);

const pushPromises = fcmTokens.map(token =>
  sendAzurePushNotification(token, {
    title: '🚨 Emergency Alert',
    body: 'Stampede reported 0.5km from your location',
    data: {
      type: 'sos_alert',
      latitude: '40.7829',
      longitude: '-73.9654'
...
const results = await Promise.allSettled([...whatsappPromises, ...pushPromises]);
*/

// Example 2: Register device for notifications
/*
await registerDeviceForNotifications(
  fcmToken, 
  userId, 
  ['emergency_alerts', 'location_nyc', 'premium_user']
);
*/
...
const recipients = users.map(user => ({
  ...
  parameters: {
    location: sosReport.location.address,
    distance: calculateDistance(user.lat, user.lng, sos.lat, sos.lng).toFixed(1),
    message: sosReport.message
  }
}));

const results = await sendBulkWhatsAppAlerts(recipients);
*/
